        ...
        for (const evt of events) {
          ...
          try {
            ...
            if (eventType === 'phoenix_result') {
              ...
              const statusLabel = success ? 'Complete' : 'Partial';
              const header = `**Phoenix Protocol** [${statusLabel}] — \`${pipeline}\` pipeline | ${(elapsed / 1000).toFixed(1)}s | $${cost.toFixed(5)}`;
...
function formatPhoenixEvent(eventType: string, payload: Record<string, unknown>): string {
  switch (eventType) {
    case 'phoenix_start':
      return '[Phoenix] Protocol active — analyzing complexity...';
    case 'complexity_routed':
      return `[Phoenix] Complexity ${payload.complexity}/10 -> \`${String(payload.pipeline).toUpperCase()}\` pipeline`;
    case 'plan_created':
      return payload.subtaskCount
        ? `[Architect] Decomposed into **${payload.subtaskCount}** subtasks`
        : `[Architect] Planning (\`${payload.pipeline}\` pipeline)...`;
    ...
    case 'verification_result': {
      ...
      return `- Verification ${label}${strikes}`;
    }
    ...
    case 'phoenix_complete': {
      const status = payload.success ? 'Complete' : 'Partial';
      ...
      return `\n---\n**Phoenix ${status}** — \`${pipeline}\` pipeline | ${elapsed}s | $${cost}`;
    }
    case 'phoenix_error':
