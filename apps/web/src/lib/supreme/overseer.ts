...
async function decomposeGoal(
  ...
  const system = [
    'You are the TITAN_OVERSEER — supreme coordinator of the Supreme Protocol inside the Titan AI IDE.',
    '',
    'You have FULL access to the user\'s workspace, all files, terminal, and development tools.',
    'The user\'s project is loaded and active. You can read, write, search, and execute anything in it.',
    '',
    'DECOMPOSITION RULES:',
    '- Break the user\'s goal into atomic subtasks that workers can execute independently',
    '- NEVER ask for clarification. NEVER say "I need more information". Plan based on what the user said.',
    '- If the user mentions ANY module, engine, feature, component by name — ASSUME IT EXISTS. Plan tasks to find and work on it.',
    '- First subtask should ALWAYS be workspace reconnaissance: search for relevant code, understand the structure',
    '- Each subtask MUST include relevantFiles with probable paths',
    '- Descriptions must be specific enough for a coder to execute without asking questions',
    '- NEVER create a subtask that says "ask the user" or "clarify requirements"',
    '',
    'Return strict JSON (no markdown wrapping, no explanation):',
    '{"nodes":[{"id":"node-1","title":"Short title","description":"Detailed description with specific instructions",',
...
async function authorizeExecution(
  ...
  const system = [
    'You are the TITAN_OVERSEER — supreme coordinator converting approved code into executable tool calls.',
    '',
    'Convert the code artifact into a precise sequence of tool calls that apply the changes to the workspace.',
    'Each step must use EXACTLY one tool with complete arguments.',
    '',
    'Available tools: read_file, write_file, edit_file, create_file, delete_file, run_command,',
    'list_directory, grep_search, glob_search, semantic_search, read_lints, web_search, web_fetch.',
    '',
    'Return strict JSON (no markdown wrapping, no explanation):',
    '{"steps":[{"id":"step-1","tool":"write_file","args":{"path":"src/x.ts","content":"complete file content"},',
    '"rationale":"Why this step","requiresApproval":true}]}',
    '',
    'RULES:',
    '- Extract ACTUAL file writes/edits from the artifact with COMPLETE file content',
    '- For edits, use edit_file with exact old_string and new_string — old_string must be unique in the file',
    '- Always add a read_lints step after file modifications to verify',
    '- No placeholders, no stubs, no TODOs in generated content',
