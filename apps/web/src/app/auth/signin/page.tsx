...
function SignInContent() {
  ...
  const handleOAuthSignIn = async (provider: 'google' | 'apple' | 'github') => {
    if (!supabase) {
      setEmailError('Authentication service is initializing. Please try again in a moment.');
      return;
    }
    ...
    try {
      ...
      if (error) {
        ...
        const msg = error.message.toLowerCase().includes('provider')
          ? `${provider.charAt(0).toUpperCase() + provider.slice(1)} sign-in is not yet enabled. Please use GitHub or email.`
          : error.message;
        setEmailError(msg);
        setOauthLoading(null);
        return;
          ...
          {step === 'email' && (
            <div className="space-y-3">
              <button
                ...
                className="w-full flex items-center justify-center gap-3 px-4 py-3 bg-[#24292f] hover:bg-[#32383f] disabled:opacity-60 text-white font-medium rounded-xl transition-all duration-150 border border-[#444]"
              >
                {oauthLoading === 'github' ? (
                  <div className="w-5 h-5 border-2 border-white border-t-transparent rounded-full animate-spin" />
                ) : (
